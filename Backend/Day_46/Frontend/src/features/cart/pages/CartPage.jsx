...
const CartPage = () => {
    ...
    const { handleGetCart, handleRemoveItem, handleUpdateQuantity, cartItems } = useCart()
            ...
            <div className="bg-[#f9f9f9] text-[#1a1c1c] antialiased min-h-screen flex flex-col font-['Manrope']">
                ...
                <main className="flex-1 flex items-center justify-center pt-24">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-xs uppercase tracking-[0.25em] text-[#777777]">Loading your bag…</p>
                    </div>
                </main>
                <Footer />
            </div>
        ...
        <div className="bg-[#f9f9f9] text-[#1a1c1c] antialiased min-h-screen flex flex-col font-['Manrope']">
            ...
            <div className="pt-24 px-6 md:px-16 lg:px-24">
                <nav className="flex items-center gap-3 text-[10px] uppercase tracking-[0.2em] text-[#888888]">
                    <span onClick={() => navigate('/')} className="hover:text-black cursor-pointer transition-colors">Home</span>
                    <span>/</span>
            ...
            <div className="px-6 md:px-16 lg:px-24 py-8 border-b border-[#e2e2e2]">
                <div className="flex items-end justify-between">
                    <div>
                        <p className="text-[10px] uppercase tracking-[0.3em] text-[#888888] mb-2">Review · Checkout</p>
                ...
                {items.length === 0 ? (
                    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-6 text-center">
                        <div className="w-24 h-24 border border-[#e2e2e2] flex items-center justify-center bg-white">
                            <span className="material-symbols-outlined text-5xl text-[#c6c6c6]">shopping_bag</span>
                        </div>
                        <div>
                            <h2 className="text-2xl font-['Newsreader'] text-[#474747] mb-2">Your bag is empty</h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 xl:gap-20">
                                    ...
                                    >
                                        ...
                                        >
                                            {imgUrl ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center">
                                                    <span className="material-symbols-outlined text-3xl text-[#c6c6c6]">image_not_supported</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="shrink-0 text-right flex flex-col justify-between">
                                            <span className="text-sm font-bold tracking-tight">
                                                {currencySymbol}{itemTotal.toLocaleString('en-IN')}
                                            </span>
                                            <span className="text-[10px] text-[#a1a1a1] uppercase tracking-widest">
                                                {currencySymbol}{item.price?.amount?.toLocaleString('en-IN')} ea.
                                            </span>
                                        </div>
                                    </div>
                                )
                        ...
                        <div className="lg:col-span-1">
                            <div className="sticky top-28 border border-[#e2e2e2] bg-white p-8 flex flex-col gap-6">
                                <h2 className="text-[10px] uppercase tracking-[0.3em] text-[#888888]">Order Summary</h2>

                                {/* Line items */}
                                <div className="flex flex-col gap-3">
                                    <div className="flex justify-between text-[12px] text-[#474747]">
                                        <span>Subtotal ({items.length} {items.length === 1 ? 'item' : 'items'})</span>
                                        <span className="font-medium">{currencySymbol}{subtotal.toLocaleString('en-IN')}</span>
                                    </div>
                                    <div className="flex justify-between text-[12px] text-[#474747]">
                                        <span>Shipping</span>
                                    ...
                                    {shipping > 0 && (
                                        <p className="text-[10px] text-[#a1a1a1]">
                                            Add {currencySymbol}{(999 - subtotal + 1).toLocaleString('en-IN')} more for free shipping
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="flex justify-between items-baseline">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[9px] uppercase tracking-widest text-[#a1a1a1]">{currency} · Incl. taxes</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 text-center">
                                    {[
                                        ...
                                        <div key={item.label} className="flex flex-col items-center gap-1.5">
                                            <span className="material-symbols-outlined text-[18px] text-[#474747]">{item.icon}</span>
                                            <span className="text-[8px] uppercase tracking-[0.15em] text-[#777777] leading-tight">{item.label}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
